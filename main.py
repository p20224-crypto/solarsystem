import streamlit as st

st.title('태양계 행성들의 위성')

# 사용자 입력 받기
selected_planet = st.selectbox('행성을 선택해주세요:', [
    '수성', '금성', '지구', '화성', '목성', '토성', '천왕성', '해왕성'
])

# 위성 설명 (더 자세히)
planet_data = {
    '수성': {
        '특징': '태양에서 가장 가깝고 태양계에서 가장 작은 행성.',
        '대표적인_위성': '없다.',
    },
    '금성': {
        '특징': '지구와 유사한 크기이지만, 두꺼운 이산화탄소 대기와 그로 인한 극단적인 온실효과가 있다.',
        '대표적인_위성': '없다.',
    },
    '지구': {
        '특징': '생명체가 존재하며 대기가 질소와 산소로 구성되어 있다.',
        '대표적인_위성': '달',
        '위성_특징': '지구에서 가장 가까운 천체',
    },
    '화성': {
        '특징': '매우 희박한 이산화탄소 대기, 낮은 중력, 물의 흔적이 있다.',
        '대표적인_위성': '포보스, 데이모스',
        '위성_특징': {
            '포보스': '화성을 공전하고 있는 두 위성 중 가까이 있는 위성.',
            '데이모스': '미국의 천문학자인 아사프 홀이 발견했다.',
        },
    },
    '목성': {
        '특징': '태양계에서 가장 큰 거대 가스 행성으로, 강력한 자기장과 빠른 자전, 거대한 폭풍 대적점이 특징이다.',
        '대표적인_위성': '이오, 유로파, 가니메데, 칼리스토',
        '위성_특징': {
            '이오': '평균 반지름이 약 1,800km로 달과 비슷한 크기이다.',
            '유로파': '표면 아래 거대한 지하 바다가 존재할 가능성이 높아 생명체 후보지로 주목받는다.',
            '가니메데': '반지름 2,634km로 태양계에서 가장 큰 위성이다.',
            '칼리스토': '태양계에서 세 번째로 큰 위성으로, 표면 충돌구가 많다.',
        },
    },
    '토성': {
        '특징': '거대한 고리와 많은 위성을 가진 행성.',
        '대표적인_위성': '타이탄, 엔셀라두스, 미마스',
        '위성_특징': {
            '타이탄': '두꺼운 대기와 액체 메탄 호수가 존재한다.',
            '엔셀라두스': '주로 얼음과 암석으로 구성된 위성.',
            '미마스': '독특한 밝기 차이를 가진 위성.',
        },
    },
    '천왕성': {
        '특징': '푸른색을 띠는 가스 행성으로, 자전축이 거의 옆으로 누워있다.',
        '대표적인_위성': '미란다, 아리엘, 우룸, 오베론',
        '위성_특징': {
            '미란다': '표면 지형이 매우 다양하고 기괴한 모양을 가짐.',
            '아리엘': '얼음과 암석으로 이루어진 위성.',
            '우룸': '표면에 충돌구가 많음.',
            '오베론': '가장 바깥쪽에 있는 큰 위성 중 하나.',
        },
    },
    '해왕성': {
        '특징': '푸른색을 띠는 가스 행성, 강력한 바람과 폭풍이 특징.',
        '대표적인_위성': '트리톤, 넵투누스, 나이아드',
        '위성_특징': {
            '트리톤': '역행 궤도를 돌고 있으며, 질소 분출이 관찰됨.',
            '넵투누스': '작은 위성, 얼음과 암석으로 구성.',
            '나이아드': '해왕성에 가까운 궤도를 가진 작은 위성.',
        },
    },
}

if st.button('특징 생성'):
    if selected_planet in planet_data:
        특징 = planet_data[selected_planet].get('특징', '정보 없음')
        대표적인_위성 = planet_data[selected_planet].get('대표적인_위성', '정보 없음')
        위성_특징 = planet_data[selected_planet].get('위성_특징', None)

        st.write(f"**특징**: {특징}")
        st.write(f"**대표적인 위성**: {대표적인_위성}")

        # 위성_특징이 dict이면 문단 나누어 표시
        if isinstance(위성_특징, dict):
            for 위성, 설명 in 위성_특징.items():
                st.markdown(f"**{위성}**  \n{설명}\n")
        else:
            st.write(위성_특징 or "위성 관련 상세 정보가 없습니다.")
    else:
        st.write(f"{selected_planet}에 대한 정보가 없습니다.")
